...
class DrawManager {
    ...
    getDrawElements(ctx, objectName, position, delta = 0, animationOffset = 0) {
        ...
        const getDrawElements = Math.floor(this.render_torch_time + animationOffset / 8);
        const { pos, size, dest, frames, offset } = this.getObject(objectName);
        const drawFrame = frames ? getDrawElements % frames : 0;
        // ctx.globalAlpha = 1;
        ctx.drawImage(SPRITESHEETS.dungeon, pos.x + size.x * drawFrame, pos.y, size.x, size.y, position.x - offset.x, position.y - offset.y, dest.x, dest.y);
    }
    drawHalf(ctx, objectName, position, half = false) {
        const { pos, size, dest, offset } = this.getObject(objectName);
        // void ctx.drawImage(image, sx, sy, sWidth, sHeight, dx, dy, dWidth, dHeight);
        ctx.drawImage(SPRITESHEETS.dungeon, half ? pos.x + size.x / 2 : pos.x, pos.y - size.y, size.x / 2, size.y, half ? position.x + 48 - offset.x : position.x + 20 - offset.x, position.y - 8, dest.x - 12, dest.y);
    }
