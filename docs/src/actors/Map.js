...
class Map {
    constructor(tileSize = 23) {
        ...
        this.currentLevel = GameMaster_1.myGameMaster.currentLevel;
        const rows = GameMaster_1.myGameMaster.myDungeonMap.trim().split("\n");
    ...
    drawWall(ctx, i, j) {
        DrawManager_1.myDrawManager.getDrawElements(ctx, "wall", this.tilePos(i, j));
    }
    drawFloor(ctx, i, j) {
        DrawManager_1.myDrawManager.getDrawElements(ctx, "floor", this.tilePos(i, j));
    }
    drawTorch(delta, ctx, i, j) {
        this.drawWall(ctx, i, j);
        const animationOffset = Math.floor(Math.random() * 8);
        DrawManager_1.myDrawManager.getDrawElements(ctx, "torch", this.tilePos(i, j), delta, animationOffset);
    }
    keyboardEvent() { }
    update() { }
    draw(delta, ctx) {
