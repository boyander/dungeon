...
const ChestManager_1 = require("./ChestManager");
...
class Skeleton {
    ...
    draw(delta, ctx) {
        ...
        GameMaster_1.myGameMaster.skeletonPosition = this.position;
        ChestManager_1.myChestManager.checkChestsStatus();
    }
    update(deltaSeconds) {
        if (GameMaster_1.myGameMaster.isSkeletonDead && !GameMaster_1.myGameMaster.debug) {
            this.currentSequence = "dead";
            this.speed = { x: 0, y: 0 };
            this.framePos = 5;
        }
        else if (GameMaster_1.myGameMaster.allChestsOpen && this.currentSequence !== "dance") {
    ...
    open() {
        let distance = 0;
        ChestManager_1.myChestManager.chests.forEach((ori) => {
            distance = 0;
            distance = Math.sqrt(Math.pow(this.position.x - ori.position.x, 2) +
                Math.pow(this.position.y - ori.position.y, 2));
            // console.log(myChestManager.latestOpenedChest);
            if (distance < 30 && !ori.isChestOpen) {
                switch (ChestManager_1.myChestManager.latestOpenedChest) {
                    case "":
                        ori.isChestOpen = true;
                        ChestManager_1.myChestManager.latestOpenedChest = ori.value;
                        break;
                    case ori.value:
                        ori.isChestOpen = true;
                        ChestManager_1.myChestManager.latestOpenedChest = "";
                        break;
                    default:
                        ChestManager_1.myChestManager.chests.forEach((e) => (e.isChestOpen = false));
                        ChestManager_1.myChestManager.latestOpenedChest = "";
                        ori.isChestOpen = true;
                        ChestManager_1.myChestManager.latestOpenedChest = ori.value;
                        break;
    ...
    drawShadow(ctx) {
        if (this.currentSequence === "dance")
            GameMaster_1.myGameMaster.viewField++;
        ...
        maskCtx.arc(this.position.x, this.position.y, GameMaster_1.myGameMaster.viewField, 0, 2 * Math.PI);
        maskCtx.fill();
